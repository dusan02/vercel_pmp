...
module.exports = {
  apps: [
    ...
    },
    {
      ...
      error_file: path.join(__dirname, "logs", "pm2", "cron-backfill-analysis-error.log"),
      out_file: path.join(__dirname, "logs", "pm2", "cron-backfill-analysis-out.log"),
      log_date_format: "YYYY-MM-DD HH:mm:ss Z",
      // Daily at 04:00 UTC — low traffic period, backfills 10 tickers per run
      cron_restart: "0 4 * * *",
      autorestart: false,
    },
    {
      name: "cron-earnings-blog",
      script: "scripts/trigger-earnings-blog.ts",
      interpreter: "/var/www/premarketprice/node_modules/.bin/tsx",
      cwd: __dirname,
      instances: 1,
      exec_mode: "fork",
      ...
      error_file: path.join(__dirname, "logs", "pm2", "cron-earnings-blog-error.log"),
      out_file: path.join(__dirname, "logs", "pm2", "cron-earnings-blog-out.log"),
      log_date_format: "YYYY-MM-DD HH:mm:ss Z",
      // Every Monday at 09:00 UTC — generates weekly earnings blog post
      cron_restart: "0 9 * * 1",
      autorestart: false,
    },
  ],
