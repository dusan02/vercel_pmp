...
**P0-lite: Twitter/X posting — opravené app-side, externý blocker (2026-09-21).** Dva app bugy fixnuté a deploynuté: (1) `NEXT_PUBLIC_APP_URL` chýbal v prod `.env` → OG fetch išiel na `localhost:3000` → 404 → throw pred X API — doplnené `NEXT_PUBLIC_APP_URL=http://127.0.0.1:3001` + `pm2 restart`; (2) `v1.uploadMedia` vracal 402 (media upload nie je v aktuálnom API tieri) → `socialDistributorService` uploaduje médium best-effort a pri zlyhaní postne text-only tweet (analysis link sa aj tak rozbalí do OG karty cez `/analysis/[ticker]/opengraph-image`). **Zostávajúci blocker: `POST /2/tweets` vracia 402 `credits-depleted`** — X API post kredity účtu sú za billing periódu minuté; `GET /2/users/me` funguje (`@premarketprice`), credentials sú OK. Vyžaduje akciu v X Developer Portal (usage/billing → počkať na reset alebo upgrade tieru) — app-side nie je čo fixovať. Manuálny test: `POST /api/cron/post-social` s Bearer `CRON_SECRET_KEY` (nie `CRON_SECRET` — `cronAuth` akceptuje obe, ale prod má len KEY variant). Redis `social:quota:daily:*`/`social:posted:*` sa plnia až po úspešnom tweete — failed run nič nekonzumuje. Známe chyby (nechávame): `updateCronStatus` "success" ≠ tweet publikovaný; info logy strippuje `removeConsole`; dedup lock sa setuje až po poste (race okno). Metrika: tweet→Analysis CTR, nie follower count.
