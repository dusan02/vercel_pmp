...
async function main() {
  ...
  stocks.forEach(s => {
    const ageMs = etNow.getTime() - (s.lastPriceUpdated?.getTime() ?? 0);
    ...
    const isStale = !isFrozen && (s.lastPrice ?? 0) > 0 && ageMs > thresholdMin * 60_000;
    
    console.log(`${s.symbol}:`);
    console.log(`  Cena: ${(s.lastPrice ?? 0).toFixed(2)}`);
    console.log(`  Posledná aktualizácia: ${s.lastPriceUpdated?.toISOString() ?? "N/A"}`);
