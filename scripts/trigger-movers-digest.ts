/**
 * Trigger movers-digest cron locally (VPS single source of truth)
 * Run: npx tsx scripts/trigger-movers-digest.ts
 */
...
async function main() {
  ...
  const url = `${baseUrl}/api/cron/movers-digest`;
  console.log(`🚀 Triggering movers-digest...`);
  ...
  console.log('✅ movers-digest completed');
  console.log(JSON.stringify(json, null, 2));
}

main().catch((e) => {
  console.error('❌ trigger-movers-digest failed:', e);
  process.exit(1);
});
