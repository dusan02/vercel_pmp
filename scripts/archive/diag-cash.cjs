...
async function main() {
  ...
  for (const sym of tickers) {
    ...
    for (const s of stmts) {
      const cash =
        s.cashAndEquivalents !== null
          ? `$${(s.cashAndEquivalents / 1e9).toFixed(2)}B`
          : "NULL";
      const debt =
        s.totalDebt !== null ? `$${(s.totalDebt / 1e9).toFixed(2)}B` : "NULL";
      const ca =
        s.currentAssets !== null
          ? `$${(s.currentAssets / 1e9).toFixed(2)}B`
          : "NULL";
      console.log(
        `    FY${s.fiscalYear}: cash=${cash}  debt=${debt}  currentAssets=${ca}`,
      );
    }
  }
  ...
  if (FINNHUB_KEY) {
    ...
    for (const sym of tickers) {
      ...
      try {
        ...
        // Show extractSum breakdown (Strategy 2 from analysisService)
        const sumConcepts = [
          "us-gaap_CashAndCashEquivalentsAtCarryingValue",
          "us-gaap_MarketableSecuritiesCurrent",
          "us-gaap_ShortTermInvestments",
        ...
        console.log("    --- extractSum breakdown (Strategy 2) ---");
        let sumTotal = 0;
        let sumCount = 0;
        for (const sc of sumConcepts) {
          const match = bs.find((i) => i.concept === sc);
          if (match && match.value != null) {
            const v =
              match.value >= 1e9
                ? `$${(match.value / 1e9).toFixed(2)}B`
                : `$${(match.value / 1e6).toFixed(0)}M`;
            console.log(`      + ${sc} = ${v}`);
            sumTotal += match.value;
            sumCount++;
          } else {
            console.log(`      - ${sc} = null`);
          }
        ...
        const totalStr =
          sumTotal >= 1e9
            ? `$${(sumTotal / 1e9).toFixed(2)}B`
            : `$${(sumTotal / 1e6).toFixed(0)}M`;
        console.log(
          `      = TOTAL: ${totalStr} (${sumCount} concepts matched)`,
        );
        console.log("    --- all cash-related concepts ---");
        ...
        if (found.length === 0) {
          ...
          for (const f of found) {
            const valStr =
              f.value >= 1e9
                ? `$${(f.value / 1e9).toFixed(2)}B`
                : `$${(f.value / 1e6).toFixed(0)}M`;
            const match = cashConcepts.includes(f.concept)
              ? "✅ MATCHED"
              : "❌ NOT EXTRACTED";
