// Automatic blog report scheduler - Core class
import { getCachedData } from './redis';

export interface ScheduledBlogConfig {
  enabled: boolean;
...
export class BlogScheduler {
  ...
  private async saveReportToFile(reportUrl: string): Promise<void> {
    try {
      const response = await this.safeFetch(reportUrl);
  ...
  private async publishToWebhook(reportUrl: string): Promise<void> {
    try {
      ...
      const response = await this.safeFetch(reportUrl + '?format=json');
      const reportData = await response.json();
      
      // Send to webhook (could be WordPress, Ghost, etc.)
      await this.safeFetch(this.config.webhookUrl, {
        method: 'POST',
        headers: {
