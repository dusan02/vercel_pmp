...
export function getSectorFromSic(sicCode: string | number | undefined | null): string | null {
  ...
  if ((code >= 3570 && code <= 3579) || // Computer and Office Equipment
      (code >= 3600 && code <= 3699) || // Electronic & Other Electrical Equipment (incl Semiconductors)
      (code >= 4800 && code <= 4899) || // Communications
      (code >= 7370 && code <= 7379) || // Computer Programming, Data Processing, etc.
      code === 8711) {                  // Engineering Services (often Tech)
    return 'Technology';
  }
  ...
  if ((code >= 3711 && code <= 3716) || // Motor Vehicles
      (code >= 4400 && code <= 4499) || // Water Transportation (Cruises like CCL)
      (code >= 4500 && code <= 4599) || // Air Transportation
      (code >= 5200 && code <= 5999) || // Retail Trade (except Food)
      (code >= 7000 && code <= 7099) || // Hotels & Lodging
      (code >= 7800 && code <= 7999)) { // Motion Pictures/Amusement
    return 'Consumer Cyclical';
  }

  // 5. Consumer Defensive (Consumer Staples)
  ...
  if ((code >= 1500 && code <= 1799) || // Construction
      ...
      (code >= 3500 && code <= 3599 && !(code >= 3570 && code <= 3579)) || // Industrial Machinery
      (code >= 3700 && code <= 3799 && !(code >= 3711 && code <= 3716)) || // Transportation Equipment (except Auto)
  ...
  // 8. Basic Materials
  if ((code >= 1000 && code <= 1299) || // Metal Mining (Gold/Silver/Iron)
      (code >= 1400 && code <= 1499) || // Nonmetallic Minerals
      (code >= 2400 && code <= 2499) || // Lumber & Wood
      (code >= 2600 && code <= 2699) || // Paper & Allied Products
      (code >= 2800 && code <= 2824) || // Chemicals & Allied Products (except Drugs)
      (code >= 2850 && code <= 2899) || // Paint/Chemicals
      (code >= 3200 && code <= 3299)) { // Stone, Clay, Glass, Concrete
    return 'Basic Materials';
  }

  // 9. Utilities
  if (code >= 4900 && code <= 4999) {
    return 'Utilities';
  }

  // 10. Real Estate
  if (code >= 6500 && code <= 6799) {
    return 'Real Estate';
  }
