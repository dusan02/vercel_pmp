...
export function detectSession(etNow?: Date): 'pre' | 'live' | 'after' | 'closed' {
  ...
  if (dayOfWeek === 0) { // Sunday in ET
    // US stock exchanges are closed all day Sunday. The previous 18:00+ 'after'
    // treatment (for futures/international) caused the worker to ingest Friday
    // close data on Sunday evening and bump DB lastPriceUpdated timestamps.
    // This site tracks S&P 500 stocks only, so Sunday is always closed.
    return 'closed';
  }
...
export function isMarketHoliday(date: Date): boolean {
  ...
  // Handles year wrap: New Year's Day on Saturday is observed on Friday Dec 31
  // of the PREVIOUS year (e.g. 2028-01-01 Sat -> observed 2027-12-31 Fri).
  const isObservedFixedHoliday = (m: number, d: number) => {
    // Actual holiday
    if (month === m && day === d) return true;
    // Observed Friday if holiday on Saturday
    if (dayOfWeek === 5) {
      // Same-year case: Friday observed -> actual holiday is next day (Saturday)
      if (m === month && d === day + 1) return true;
      // Year-wrap case: Dec 31 Friday observed for Jan 1 Saturday
      if (m === 1 && d === 1 && month === 12 && day === 31) return true;
    }
