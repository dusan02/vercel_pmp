...
describe('priceResolver', () => {
  ...
  describe('resolveEffectivePrice - Empty snapshot fallback (Polygon 03:30 ET clear)', () => {
    const emptySnapshot: PolygonSnapshot = {
      ticker: 'AAPL'
      // Polygon clears snapshots daily at 03:30 ET; before repopulation
      // there is no min/lastTrade/lastQuote/prevDay data at all.
    };
    ...
    it('should fall back to our own previousClose when snapshot is empty', () => {
      const preMarketET = new Date('2025-01-15T05:00:00-05:00');
      const result = resolveEffectivePrice(emptySnapshot, 'pre', preMarketET, undefined, false, 150.25);

      expect(result).not.toBeNull();
      expect(result?.price).toBe(150.25);
      expect(result?.source).toBe('regularClose');
      expect(result?.isStale).toBe(true);
    });

    it('should return null when snapshot is empty and no fallback previousClose is provided', () => {
      const result = resolveEffectivePrice(emptySnapshot, 'pre', mockETNow);

      expect(result).toBeNull();
    });

    it('should prefer snapshot prevDay.c over the fallback previousClose', () => {
      const snapshotWithPrevDay: PolygonSnapshot = {
        ticker: 'AAPL',
        prevDay: { c: 149.50 }
      };

      const result = resolveEffectivePrice(snapshotWithPrevDay, 'pre', mockETNow, undefined, false, 150.00);

      expect(result).not.toBeNull();
      expect(result?.price).toBe(149.50); // snapshot prevDay.c wins over the fallback
      expect(result?.source).toBe('regularClose');
    });

    it('should not use fallbackPreviousClose in live session', () => {
      const result = resolveEffectivePrice({ ticker: 'AAPL' }, 'live', mockETNow, undefined, false, 150.00);

      expect(result).toBeNull();
    });

    it('should use mid-price (bid/ask) for lastQuote instead of raw bid', () => {
      const snapshot: PolygonSnapshot = {
        ticker: 'AAPL',
        lastQuote: {
          p: 150.00, // bid
      ...
      expect(result).not.toBeNull();
      expect(result?.price).toBeCloseTo(150.05, 5);
      expect(result?.source).toBe('lastTrade');
    });
  });
