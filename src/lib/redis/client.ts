...
try {
    ...
    // Priority 2: Explicit REDIS_URL always wins over the local default —
    // USE_LOCAL_REDIS only forces local Redis when REDIS_URL is not set.
    // (Previously USE_LOCAL_REDIS overrode an explicit REDIS_URL, pointing
    // the app at 6379 while production Redis runs on 6380.)
    else if (process.env.REDIS_URL) {
        redisUrl = process.env.REDIS_URL;
        if (process.env.USE_LOCAL_REDIS === 'true') {
            console.log('🔍 Using REDIS_URL (explicit):', redisUrl);
        }
    }
    // Priority 3: Default to local Redis when nothing is configured
    ...
    if (redisUrl) {
        ...
        // NOTE: the timeout only LOGS — it must not null the client, because a
        // slow connect (busy event loop at boot) that succeeds at t+5s is still
        // perfectly usable. Nulling it here permanently broke the health check
        // in route-bundle module instances (client nulled, socket actually open).
        if (!redisClient.isOpen) {
            ...
            Promise.race([connectPromise, timeoutPromise]).catch((err: any) => {
                console.warn('⚠️ Redis connect slower than 3s (continuing in background):', err.message || err);
            });
        }
    } else {
        redisClient = null;
