...
export const REDIS_KEYS = {
    ...
    rankZScore: (date: string, session: string) => `rank:zscore:${date}:${session}`,
    rankRVOL: (date: string, session: string) => `rank:rvol:${date}:${session}`,
...
export const REDIS_TTL = {
    ...
    PRE_AFTER: 300, // 5 minutes for pre/after hours — must cover REST_INTERVAL (5min) so non-premium keys don't expire between ingest cycles
