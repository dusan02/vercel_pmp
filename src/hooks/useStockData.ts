...
export function useStockData({ initialData = [], favorites }: UseStockDataProps) {
  ...
  // Server broadcasts top 100 + favorites via socket.io (server.ts). Updates are
  // merged silently into stockData; HTTP polling above stays as the fallback
  // for tickers the stream does not cover and when WS is unavailable.
  const liveConnected = usePriceStream(favorites, useCallback((updates: PriceUpdate[]) => {
    setStockData(prev => {
      const map = new Map(prev.map(s => [s.ticker, s]));
      for (const u of updates) {
        const existing = map.get(u.ticker);
        if (!existing) continue; // stream covers known tickers only — don't grow the list here
        map.set(u.ticker, {
          ...existing,
          currentPrice: u.currentPrice ?? existing.currentPrice,
          closePrice: u.previousClose ?? existing.closePrice,
          percentChange: u.percentChange ?? existing.percentChange,
          marketCap: u.marketCap || existing.marketCap,
          marketCapDiff: u.marketCapDiff || existing.marketCapDiff,
          lastUpdated: new Date(u.timestamp || Date.now()).toISOString(),
        });
      }
      return Array.from(map.values());
    });
  ...
  const fetchRemainingStocksData = useCallback(async () => {
    ...
    const loadInBackground = async () => {
      ...
      try {
        ...
        const response = await fetchWithRetry(
          `/api/stocks?getAll=true&project=${project}&sort=marketCap&order=desc&limit=3000&t=${Date.now()}`,
          3,
          2000
        );
        ...
        if (response && response.ok) {
          ...
          if (result.data && result.data.length > 0) {
            ...
            // Empty payload — clear the loading flag, otherwise it sticks forever
            isLoadingRemainingRef.current = false;
            setLoadingStates(prev => ({ ...prev, remainingStocks: false }));
          }
        } else {
          // Failed response (429/500/null after retries) — clear the loading flag
          isLoadingRemainingRef.current = false;
          setLoadingStates(prev => ({ ...prev, remainingStocks: false }));
        }
      } catch (error) {
  ...
  useEffect(() => {
    ...
    const initialDelay = setTimeout(() => {
      ...
      refreshInterval = setInterval(() => {
        ...
        fetchAndMergeStocks(
          `/api/stocks?tickers=${getProjectTickers(project, 50).join(',')}&project=${project}&limit=50&t=${Date.now()}`,
          'top50Stocks',
  ...
  }, [fetchFavoritesData, fetchAndMergeStocks]);
  ...
  useEffect(() => {
    const currentTickers = new Set(
      favoritesTickersString ? favoritesTickersString.split(',') : []
    );
    const previousTickers = new Set(
      previousFavoritesRef.current ? previousFavoritesRef.current.split(',') : []
    );

    // Only fetch if:
    // 1. This is the initial load (no previous snapshot)
    // 2. At least one NEW ticker was added (set comparison — string length
    //    comparison misfires when a short ticker is replaced by a long one)
    // Don't fetch if favorites were only removed (to avoid flickering)
    const hasAddition = [...currentTickers].some(t => !previousTickers.has(t));
    const isInitialLoad = previousFavoritesRef.current === '';

    if (favorites.length > 0 && (isInitialLoad || hasAddition)) {
      ...
      previousFavoritesRef.current = favoritesTickersString;
