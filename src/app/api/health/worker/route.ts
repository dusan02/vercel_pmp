...
export async function GET(request: NextRequest) {
  try {
    ...
    const { detectSession } = await import('@/lib/utils/timeUtils');
    const session = detectSession(etNow);
    const isLiveMarket = session === 'live';
    ...
    // Only enforce freshness threshold during LIVE market hours
    // During Pre/After/Closed, low freshness is expected/normal
    const isFreshnessHealthy = isLiveMarket ? metrics.percentage.fresh > 80 : true;
