...
export async function GET(request: NextRequest) {
  try {
    ...
    // Security check for production — fail-closed when no secret configured
    if (process.env.NODE_ENV === 'production') {
      const metricsSecret = process.env.METRICS_SECRET_KEY;
      if (!metricsSecret) {
        return NextResponse.json(
          { error: 'Metrics auth not configured: set METRICS_SECRET_KEY' },
          { status: 503 }
        );
      }
      const authHeader = request.headers.get('authorization');
      const expectedAuth = `Bearer ${metricsSecret}`;
