...
const SwipeableTableRow = dynamic(
  () => import('@/components/SwipeableTableRow').then(mod => ({ default: mod.SwipeableTableRow })),
  { ssr: false, loading: () => null }
);
...
// Hooks and utilities - these are usually safe to import statically
...
// Import PriceUpdate type from types file instead of websocket-server (which has server-side code)
...
export default function HomePage() {
  ...
  return (
    <>
      ...
      <Suspense fallback={<div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', height: '100vh', backgroundColor: '#000', color: '#fff' }}>Loading...</div>}>
