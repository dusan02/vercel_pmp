...
const PageControls = dynamic(
  () => import('@/components/PageControls').then(mod => ({ default: mod.PageControls })),
  { ssr: false, loading: () => null }
);
...
const HeatmapPreview = dynamic(
  () => import('@/components/HeatmapPreview').then(mod => ({ default: mod.HeatmapPreview })),
  { ssr: false, loading: () => null }
);
          ...
          >
            <PullToRefresh onRefresh={loadData}>
              ...
              <main className="container" role="main">
                ...
                {/* Page Controls (Sections & Dark Mode) */}
                <PageControls 
                  preferences={preferences}
                  onToggleSection={(key) => savePreferences({ [key]: !(preferences[key] ?? true) })}
                  onThemeChange={(newTheme) => savePreferences({ theme: newTheme })}
                />
