...
function StatPill({ label, value, highlight }: { label: string; value: number | null; highlight?: 'green' | 'red' | 'gray' }) {
    ...
    return (
        <div className="flex flex-col items-center px-3 py-2 bg-gray-50 dark:bg-gray-800/60 rounded-lg min-w-[64px]">
            <span className={`text-sm font-bold tabular-nums ${col}`}>
        ...
        <div className="space-y-4">
            ...
            <div className="flex flex-wrap gap-2 items-end">
                ...
                <div className="flex flex-col mr-2">
                    <span className="text-[10px] text-gray-400 uppercase tracking-wide">Current</span>
                    <span className="text-2xl font-bold tabular-nums" style={{ color: cfg.color }}>
                        {current !== null && current !== undefined ? `${current.toFixed(2)}×` : '—'}
                    </span>
                </div>
            ...
            {/* Legend */}
            <div className="flex flex-wrap gap-4 text-[10px] text-gray-400 dark:text-gray-500">
                <span className="flex items-center gap-1.5"><span className="w-3 h-px border-t border-dashed border-emerald-500 inline-block" /> P10 — historically cheap</span>
                ...
                <span className="flex items-center gap-1.5"><span className="w-3 h-px border-t border-dashed border-red-500 inline-block" /> P90 — historically expensive</span>
                <span className="flex items-center gap-1.5 ml-auto text-gray-300 dark:text-gray-600">{stats?.count ?? 0} data points (10Y)</span>
            </div>
        </div>
    );
}
