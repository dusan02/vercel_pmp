        ...
        <div data-html2canvas-ignore="true" className="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700 p-4">
            <div className="flex flex-col md:flex-row md:items-center gap-4">
                <div className="flex items-center gap-2 flex-wrap">
                    ...
                    {compareWith && (
                        <button onClick={onRemoveComparison} disabled={loadingCompare} aria-label="Remove comparison" className="ml-auto md:ml-4 text-xs text-gray-500 hover:text-red-500 transition-colors disabled:opacity-50 focus-visible:ring-2 focus-visible:ring-blue-500 rounded">
                            ✕ Remove
                        </button>
                    )}
                    {compareError && (
                    ...
                    <div className="flex-1">
                        ...
                        {peers && peers.length > 0 && (
                            <div className="flex flex-wrap gap-2 mt-2">
                                <span className="text-xs text-gray-500">Sector peers:</span>
                                {peers.map((p) => (
                                    <button
                                        key={p}
