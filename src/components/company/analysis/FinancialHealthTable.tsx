...
function buildMetrics(data: AnalysisData, sec: AnalysisData | null, cw: string) {
    ...
    const pbRatio  = hasNegEquity ? null : (fh?.pbRatio ?? ((mcap != null && bs?.totalEquity != null && bs.totalEquity > 0) ? mcap / bs.totalEquity : null));
    const sHasNegEquity = sbs?.totalEquity != null && sbs.totalEquity <= 0;
    const spbRatio = sHasNegEquity ? null : (sfh?.pbRatio ?? ((smcap != null && sbs?.totalEquity != null && sbs.totalEquity > 0) ? smcap / sbs.totalEquity : null));
...
export function FinancialHealthTable({ ticker, data, compareWith, secondaryData }: Props) {
    ...
    return (
        <div className="bg-transparent">
            ...
            {/* Score legend (summary scores are rendered server-side above) */}
