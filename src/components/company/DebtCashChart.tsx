...
export default function DebtCashChart({ statements }: DebtCashChartProps) {
    ...
    const chartData = useMemo(() => {
        ...
        return sorted.map(s => {
            // Missing balance-sheet data must stay null — coercing to 0 made
            // empty periods look like real zeros and corrupted netDebt.
            const hasCash = s.cashAndEquivalents != null;
            const hasDebt = s.totalDebt != null;
            const cashVal = hasCash ? s.cashAndEquivalents! / 1e6 : null;
            const debtVal = hasDebt ? s.totalDebt! / 1e6 : null;
            const netDebtVal = (hasDebt || hasCash) ? (debtVal ?? 0) - (cashVal ?? 0) : null;
    ...
    const yMin = useMemo(() => {
        const allVals = chartData.flatMap(d => {
            ...
            // Only include the netDebt series when it is actually displayed —
            // a hidden series must not stretch the axis.
            if (selectedMetrics.includes('netDebt')) vals.push(d.netDebt);
    ...
    // Both values must exist for a meaningful net-cash statement
